...
### #번호 마커 규칙 (반드시 준수)
- #번호 마커는 해당 자막(element)이 화면에 등장하는 정확한 시점을 나레이션 안에 표시
- **마커 수 = elements 배열의 총 개수와 반드시 일치**
  - elements가 2개(heading + bullets) → 나레이션에 #1, #2 반드시 포함
  - elements가 3개 → #1, #2, #3 반드시 포함
  - bullets/circles의 items 개수와 무관, elements 개수 기준
- 마커는 해당 자막이 등장하기 직전 문장 뒤에 삽입
- 나레이션 마지막에 남은 마커가 몰리지 않도록 자연스럽게 분산

나레이션 예시 (elements 3개: question + heading + bullets):
"오늘은 생애 주기적 진로 관점에 대해 알아봅니다. #1 진로는 단 한 번의 선택이 아닙니다. #2 전직, 실업, 재취업, 은퇴처럼 삶 속에서 계속 이어집니다. #3 각 사건마다 어떤 태도를 보이는지가 중요합니다."

elements 4개(heading + circles 4항목)일 때 → #1 #2 #3 #4 모두 포함 필수
