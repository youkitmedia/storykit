...
### bullets (항목 나열)
- 역할: 병렬 개념, 특성, 핵심 포인트
- 규칙: **2~3개** (최대 3개, 4개 이상 금지), 각 20자 이내
- 사용: 분류, 특성, 핵심 포인트
...
### infographic (인포그래픽 — 시각 중심 콘텐츠)
- 역할: 데이터/통계/관계/흐름을 시각적으로 표현
- subtype 필수 선택:
  - "icons": 3~5개 개념을 아이콘+텍스트로 병렬 표시 (일반 개념 비교)
  - "flow": 3~5개 단계를 화살표로 연결 (프로세스, 인과관계)
  - "chart": 3~5개 데이터를 막대차트로 표시 (수치, 통계, 비율)
- text: 인포그래픽 제목 (선택)
- items: 각 항목 텍스트 (chart의 경우 "항목명 숫자%" 형식 권장)
- 사용 조건: 데이터/통계/단계/관계를 시각적으로 표현할 때
- 예: { type:"infographic", subtype:"flow", text:"학습 설계 프로세스", items:["분석","설계","개발","실행","평가"] }
...
#### 레이아웃별 나레이션 분량
- **중앙 배치 (question/emphasis/title_only)**: 100~200자
  - 화면 텍스트 1~2개, 임팩트 있는 짧은 나레이션
- **우측 배치 (concept/summary) — bullets/circles 2~3개**: 200~350자
  - 각 항목이 나레이션의 특정 문장과 1:1 매핑
- **우측 배치 — bullets/infographic 3개 이상 또는 인포그래픽**: 350~600자
  - 항목 수만큼 나레이션을 길게 작성, 각 항목 등장 시 관련 문장 충분히

#### bullets/circles/infographic 나레이션 작성 원칙 (핵심)
각 항목(item)은 나레이션의 특정 문장에서 추출됩니다:
- **첫 번째 항목**: 나레이션의 첫 번째 의미 단위 문장에서 핵심어 추출
- **두 번째 항목**: 나레이션의 두 번째 의미 단위 문장에서 핵심어 추출
- **세 번째 항목**: 나레이션의 세 번째 의미 단위 문장에서 핵심어 추출

**올바른 예시** (bullets 3개, 나레이션 350자):
items: ["인물 관계 분석", "진로 목표 구체화", "어려움 공동 탐색"]
narration: "영화 속 인물들과 그들의 직업적 관계를 분석하면서, 여러분 자신의 진로 목표를 구체화하고, 진로를 선택할 때 부딪히는 어려운 점들을 함께 탐색해 나가는 수업입니다. #1 이번 주차 강의에서는 여러분이 가졌던 직업에 대한 고정관념을 조금 깨드리고 싶습니다. #2 우리가 영화를 통해 진로를 탐색하는 세 가지 방법입니다. 첫째는 영화 속 인물 관계를 분석하는 것이고, #3 둘째는 자신의 진로 목표를 구체화하는 것이며, 셋째는 진로의 어려움을 함께 탐색하는 것입니다."

여기서:
- #1 = heading 등장 (나레이션 첫 문장 이후)
- #2 = bullets 전체 등장 직전
- 각 bullets 항목 = 나레이션의 특정 문장 내용을 요약한 핵심어

#### #마커 배정 (엄격히 준수)
- heading → #1 (나레이션 시작 직후)
- bullets/circles/infographic → 다음 번호 마커 1개 (항목 등장 직전)
- emphasis/question → 다음 번호 마커 1개
- **마커 수 = elements 배열 총 개수** (절대 규칙)
- **금지**: "...입니다. #1 #2 다음..." — 마커 2개 연속 배치 (각 마커 사이에 반드시 1문장 이상 삽입)
      ...
      "slide": {
        ...
        "elements": [
          ...
          },
          {
            ...
            "order": 2,
            "type": "bullets",
            "items": ["항목1 (20자 이내)", "항목2", "항목3 (최대 3개)"]
          }
        ]
      },
      "narration": "강사 나레이션 200~300자. #1 heading 등장 (나레이션 시작 직후). 본문 설명 충분히 작성. #2 bullets/infographic 등장 직전. 마무리 문장으로 자연스럽게 연결."
    },
    {
      "page_id": "2",
      "slide": {
        "title": "학습 설계 5단계",
        "subtitle": "| ADDIE 모델의 실제 적용",
        "layout": "concept",
        "elements": [
          {"id":"el-1","order":1,"type":"heading","text":"ADDIE 설계 모델"},
          {"id":"el-2","order":2,"type":"infographic","subtype":"flow","text":"5단계 프로세스","items":["분석","설계","개발","실행","평가"]}
        ]
      },
      "narration": "인포그래픽 예시. #1 제목 등장. 설명. #2 인포그래픽 등장 직전."
    }
  ],
  "index": [
    {
