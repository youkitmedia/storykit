...
### 분량
- **1페이지당 200~300자** (25~40초 기준, 성인 학습 몰입도 최적)
- 성인학습 원칙: 친근하고 직접적인 구어체, 사례/경험 연결
- 스토리텔링 요소 포함: 실생활 사례, 학습자 경험 연결, 구체적 상황 묘사

### #번호 마커 — 정밀 규칙
마커는 자막/인포그래픽이 화면에 등장하는 시점입니다.

**element별 마커 배정 (엄격히 준수):**
- heading → #1 (나레이션 시작 직후, 화면 전환과 동시)
- bullets → 다음 번호 마커 1개 (items 수와 무관, bullets 전체에 1개)
- circles → 다음 번호 마커 1개 (circles 전체에 1개)
- infographic → 다음 번호 마커 1개 (인포그래픽 등장 시점)
- emphasis → 다음 번호 마커 1개
- question → 다음 번호 마커 1개

**마커 수 = elements 배열 총 개수** (절대 규칙)

**올바른 예시** (elements: [heading, bullets]):
"오늘은 소방관이라는 직업의 실제 무게에 대해 알아봅니다. #1 단순히 멋진 제복을 입은 직업이 아닙니다. 현장에서 매 순간 판단을 내려야 하는, 책임과 가치가 있는 직업이죠. 이 직업이 요구하는 것들을 함께 살펴봅시다. #2 첫째는 현장 판단력, 둘째는 팀워크, 셋째는 사명감입니다."

**잘못된 예시** (금지):
"...입니다. #1 #2 다음 차시에서..." — 마커 2개가 붙어서 등장 (설명 없이 연속 배치 금지)
